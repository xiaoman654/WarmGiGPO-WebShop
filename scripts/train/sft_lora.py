...
Each row contains a `messages` field:

user: instruction + history + current observation
assistant: target WebShop action or `<think>...</think><action>...</action>`

This script masks non-assistant tokens and trains on assistant turns. Two-turn
rows and multi-turn rows are both supported.
"""
...
def encode_sample(
    ...
    for idx, message in enumerate(messages):
        ...
        answer_end = len(tokenizer(upto_text, add_special_tokens=False)["input_ids"])
        answer_start = max(cursor, min(answer_start, len(full_ids)))
        answer_end = max(answer_start, min(answer_end, len(full_ids)))
        labels[answer_start:answer_end] = full_ids[answer_start:answer_end]
        cursor = answer_end
    if appended_eos and any(label != IGNORE_INDEX for label in labels):
        labels[-1] = full_ids[-1]
