...
for size in 500 2k; do
  python scripts/train/merge_lora_adapter.py \
    --base-model "$MODEL_DIR" \
    --adapter "outputs/sft/qwen25_1p5b_webshop_lora_verl_${size}/final_adapter" \
    --output-dir "outputs/sft/qwen25_1p5b_webshop_lora_verl_${size}/merged_model"
done
